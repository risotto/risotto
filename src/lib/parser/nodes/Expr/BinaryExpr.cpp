...
FunctionEntry *BinaryExpr::getFunctionEntry(Compiler *compiler) {
    ...
    return leftTypeEntry[0]->findOperator(compiler, op()->lexeme, getArgumentsTypes(compiler));
}

FunctionNotFoundError BinaryExpr::getFunctionNotFoundError(Compiler *compiler) {
    return FunctionNotFoundError(op()->lexeme, left()->getReturnType(compiler)[0]->toString(), getArgumentsTypes(compiler), op());
}
